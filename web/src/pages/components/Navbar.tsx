...
const Navbar: React.FC<Props> = ({ user }) => {
  ...
  const logout = async () => {
    await req.post(
      '/auth/logout',
      {},
      destroySession ? { params: { destroySession: 1 } } : undefined
    )
  ...
  return (
    <>
      <Layout.Header
        style={{
          background: currentTheme === 'dark' ? '#1f1f1f' : '#0088CC',
          padding: '0 20px'
        }}
      >
        <div key="logo" className="logo" style={{ marginRight: '30px' }}>
          {/\/startup/.test(pathname) ? (
            <img
              src="/teledrive-logo/logoteledrive-white.png"
              style={{ height: '24px' }}
            />
          ) : (
            <Link to="/dashboard" style={{ color: '#fff' }}>
              <img
                src={`${process.env.PUBLIC_URL}/teledrive-logo/logoteledrive-white.png`}
                style={{ height: '24px' }}
              />
            </Link>
          )}
                ...
                /\/admin/.test(pathname) ? (
                  <>
                    ...
                    >
                      <Button
                        type="link"
                        style={{ color: '#ffff', float: 'right', top: '12px' }}
                        icon={
                          <Avatar
                            src={`${apiUrl}/users/me/photo`}
                            icon={<UserOutlined />}
                          />
                        }
                      />
                  ...
                  <Button
                    type="link"
                    style={{ color: '#ffff', float: 'right', top: '16px' }}
                    icon={<DashboardOutlined />}
                    onClick={() => history.push('/dashboard')}
                  >
                    {window.innerWidth > 359 && 'Dashboard'}
                  </Button>
                )}
              </>
            ) : (
      ...
      >
        <Typography.Paragraph>Are you sure to logout?</Typography.Paragraph>
        <Form.Item help="All files you share will not be able to download once you sign out">
          <Checkbox
            checked={destroySession}
            onChange={({ target }) => setDestroySession(target.checked)}
          >
